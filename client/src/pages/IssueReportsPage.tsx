...
const isTerminalStatus = (value: string) => value === 'RESOLVED' || value === 'WONT_FIX';
      ...
      <main className="flex-1 overflow-auto bg-light-bg px-4 py-4 pb-24 dark:bg-slate-900 sm:px-6 md:pb-6">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map((issue) => {
              const responseLocked = isTerminalStatus(issue.status);
              return (
                <article key={issue.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (isTerminalStatus(nextStatus)) {
                          const textarea = e.currentTarget.closest('article')?.querySelector('textarea');
                          const adminNotes = textarea instanceof HTMLTextAreaElement ? textarea.value.trim() || null : issue.adminNotes;
                          updateIssue(issue, { status: nextStatus, adminNotes });
                          return;
                        }
                        updateIssue(issue, { status: nextStatus });
                      }}
                      className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-[#012061] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                    >
                      {Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                    </select>
                  </div>
                  ...
                  <label className="mt-3 block">
                    <span className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Admin response / Resolution note</span>
                    <textarea
                      defaultValue={issue.adminNotes || ''}
                      disabled={responseLocked || savingId === issue.id}
                      rows={2}
                      onBlur={(e) => {
                        if (responseLocked) return;
                        if ((issue.adminNotes || '') !== e.target.value.trim()) updateIssue(issue, { adminNotes: e.target.value.trim() || null });
                      }}
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 outline-none focus:border-[#f8931f] focus:ring-1 focus:ring-[#f8931f] disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 disabled:opacity-80 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:disabled:bg-slate-950 dark:disabled:text-slate-500"
                    />
                  </label>
                </article>
              );
