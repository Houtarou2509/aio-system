    ...
    <div className="space-y-4">
      ...
      {!isEntityView && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={filters.entityType || ''} onChange={e => setFilters({ ...filters, entityType: e.target.value || undefined })} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400">
            <option value="">All types</option>
            <option value="Asset">Asset</option>
            <option value="MaintenanceLog">Maintenance</option>
            <option value="User">User</option>
          </select>
          <select value={filters.action || ''} onChange={e => setFilters({ ...filters, action: e.target.value || undefined })} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400">
          ...
          <button onClick={handleExport} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            ...
            {logs.map(l => {
              ...
              return (
                <div key={l.id} className="relative pl-10">
                  ...
                  <div className={`rounded-xl border border-slate-100 dark:border-slate-700 border-l-[3px] ${conf.accent} bg-white dark:bg-slate-800 p-3 shadow-xs`}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        {l.severity === 'HIGH' && (
                          <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-200">
                            <AlertTriangle className="w-2.5 h-2.5" /> HIGH
                          </span>
                        )}
                        {l.severity === 'MEDIUM' && (
                          <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950 text-amber-600 dark:text-amber-200">
                            MED
                          </span>
                        )}
                    ...
                    {l.summary && (
                      <div className="mt-1 text-xs font-medium text-slate-700 dark:text-slate-300">{l.summary}</div>
                    )}
                    {l.field && l.field !== '*' && (
                      <div className="mt-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-900 text-xs">
                        <span className="text-slate-500 dark:text-slate-400 font-medium">{l.field}: </span>
                    ...
                    {l.oldImageUrl && (
                      <div className="mt-1.5 flex items-center gap-2">
                        <a href={l.oldImageUrl} target="_blank" rel="noopener noreferrer" className="group">
                          <img src={l.oldImageUrl} alt="Previous" className="h-10 w-10 rounded object-cover border border-slate-200 dark:border-slate-700 group-hover:border-[#f8931f] transition-colors" />
                        </a>
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      <span className="text-xs text-slate-500 dark:text-slate-400">
                        By <span className="font-medium text-slate-700 dark:text-slate-300">{(l.performedBy as any)?.username || 'system'}</span>
      ...
      {!isEntityView && meta && meta.totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 text-xs">
          <button disabled={meta.page <= 1} onClick={() => setFilters({ ...filters, page: meta.page - 1 })} className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Prev</button>
          <span className="text-slate-500 dark:text-slate-400">Page {meta.page} of {meta.totalPages}</span>
          <button disabled={meta.page >= meta.totalPages} onClick={() => setFilters({ ...filters, page: meta.page + 1 })} className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Next</button>
        </div>
      )}
    </div>
  );
