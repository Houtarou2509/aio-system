...
function StatCard({
  ...
  const colorMap = {
    ...
    red: 'bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-200',
  ...
  return (
    <div className="rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-xs">
      ...
      <p className={`text-xl font-bold ${color === 'red' ? 'text-red-600' : color === 'indigo' ? 'text-indigo-600' : 'text-slate-900 dark:text-slate-100'}`}>{value}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Purchase Price</p>
            <p className="text-sm font-bold text-slate-900 dark:text-slate-100">₱{result.purchasePrice.toLocaleString()}</p>
          </div>
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900">
            <DollarSign className="w-4 h-4 text-slate-400" />
          </div>
        </div>
        <div className="rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Annual Depreciation</p>
            <p className="text-sm font-bold text-slate-900 dark:text-slate-100">₱{result.annualDepreciation.toLocaleString()}</p>
          </div>
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900">
      ...
      {/* ─── Depreciation Chart ─── */}
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-xs">
        <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-3">Depreciation Schedule</h3>
        <div className="h-56">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-xs">
        <div className="bg-slate-50 dark:bg-slate-900 px-4 py-2.5 border-b border-slate-200 dark:border-slate-700">
          <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Depreciation Table</h3>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-900/50">
            <tr>
              <th className="text-left px-3 py-2 font-medium text-slate-500 dark:text-slate-400 text-xs">Period</th>
              <th className="text-right px-3 py-2 font-medium text-slate-500 dark:text-slate-400 text-xs">Asset Value</th>
              <th className="text-right px-3 py-2 font-medium text-slate-500 dark:text-slate-400 text-xs">Cumulative Dep.</th>
            </tr>
          </thead>
          <tbody>
            {result.schedule.map((point, i) => (
              <tr key={i} className={`border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${i === 0 ? 'font-medium' : ''}`}>
                <td className="px-3 py-1.5 text-slate-700 dark:text-slate-300">{point.label}</td>
